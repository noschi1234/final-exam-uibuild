...
function bookComponent(sub, text, title, number) {
    return `
    <div class="item">
        <div class="book">
...
rootElement.insertAdjacentHTML('afterbegin', `<div class="header">${"New York Times Bestsellers".toUpperCase()}</p>`);

let htmlCode = `<div class="container">${books.map((book, index) => bookComponent(book.sub, book.text, book.title, index+1)).join('')}</div>`

rootElement.insertAdjacentHTML('beforeend', htmlCode)